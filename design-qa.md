...
| Area | Result | Notes |
| --- | --- | --- |
| Attribute completion | Pass | Request attributes lead the list; labels carry official version-aware categories and purpose metadata; selected details expose highlighted HTML and actions. |
| Value completion | Pass | `innerHTML` leads swap strategies; after a strategy is entered, only unused modifiers are offered. |
| Hover documentation | Pass | Native hover presents heading, version support, official categories, description, bounded values, example, and a keyboard-accessible action row. |
