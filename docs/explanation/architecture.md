...
flowchart LR
    ...
    Template["HTML or Django HTML document"] --> Scanner["Shared scanner"]
...
`build-data.py` merges HTMX `2.0.10` and `4.0.0-beta5` documentation into `htmx.catalog.json`. Each canonical attribute records descriptions, available major versions, official documentation URLs, documented values, modifiers, deprecation metadata, and dynamic-name patterns. The generated file is committed and CI regenerates it to detect drift.
...
The scanner skips HTML comments, Django comments, `{% comment %}` and `{% verbatim %}` regions, and script/style bodies. It preserves Django expressions in attribute values and recognizes same-file `{% partialdef %}` and `{% partial %}` tags.

`src/extension.ts` registers providers only for `html` and `django-html`. Diagnostics are debounced after document changes, cleared when a document closes, and recomputed when `htmxTags` settings change.
...
- Partial definitions are not indexed across a workspace.
